...
export default function PrincipalDashboard() {
  ...
  const [selectedTeacherForEvidence, setSelectedTeacherForEvidence] = useState<TeacherWithStats | null>(null);
  ...
  const { data: principalTeacherWitnesses = [], isLoading: principalEvidenceLoading } = useQuery<TeacherWitnessWithFiles[]>({
    queryKey: ["/api/principal/teachers", selectedTeacherForEvidence?.id, "evidence"],
    queryFn: async () => {
      const res = await fetch(`/api/principal/teachers/${selectedTeacherForEvidence!.id}/evidence`, { credentials: "include" });
      if (!res.ok) throw new Error("Failed");
      return res.json();
    },
    enabled: !!selectedTeacherForEvidence?.id,
  });
    ...
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      <main className="max-w-7xl mx-auto py-6 px-4">
        ...
        <Tabs defaultValue="pending" className="space-y-4">
          <TabsList className={`grid w-full lg:w-auto lg:inline-grid ${isCreator ? 'grid-cols-4' : 'grid-cols-3'}`}>
                ...
                <CardContent>
                  {!selectedTeacher ? (
                    <div className="text-center py-16 text-muted-foreground">
                      ...
                      <p className="text-lg font-medium mb-2">اختر معلماً لعرض مؤشراته</p>
                      <p className="text-sm">اختر معلماً من القائمة لعرض تفاصيل مؤشراته</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {teacherIndicators.map((indicator) => {
                        ...
                        return (
                          <Card
                            key={indicator.id}
                            className={`overflow-visible transition-all hover-elevate ${
                                    ...
                                    {principalWitnesses.map((witness) => {
                                      ...
                                      return (
                                        <Card
                                          ...
                                          onClick={() => {
                                            if (witness.link) {
                                              window.open(witness.link, "_blank");
                                            } else if (witness.fileUrl) {
                                        ...
                                        >
                                          <div className="p-4 flex items-center gap-4">
                                            ...
                                            <div className="flex gap-1 shrink-0">
                                              ...
                                              >
                                                {witness.link ? <ExternalLink className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                                                {witness.link ? "فتح الرابط" : "معاينة ذكية"}
                                              </Button>
                                              <Button
                                                size="icon"
                                                variant="ghost"
          ...
          {/* Teacher Evidence Tab */}
          <TabsContent value="evidence">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <FolderOpen className="h-5 w-5" />
                  شواهد المعلمين المستقلة
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-1">
                    <h4 className="font-bold mb-3 text-sm text-muted-foreground">اختر معلم</h4>
                    <div className="space-y-2 max-h-[500px] overflow-y-auto">
                      {teachers.map((teacher) => (
                        <div
                          key={teacher.id}
                          onClick={() => setSelectedTeacherForEvidence(teacher)}
                          className={`p-3 rounded-xl cursor-pointer transition-all flex items-center gap-3 ${
                            selectedTeacherForEvidence?.id === teacher.id ? "bg-primary/10 border border-primary" : "bg-muted/30 hover-elevate"
                          }`}
                          data-testid={`evidence-teacher-${teacher.id}`}
                        >
                          <Avatar className="h-8 w-8">
                            <AvatarImage src={teacher.profileImageUrl || undefined} />
                            <AvatarFallback className="text-xs">{getInitials(teacher.firstName, teacher.lastName)}</AvatarFallback>
                          </Avatar>
                          <div className="text-right flex-1 min-w-0">
                            <p className="font-medium text-sm truncate">{teacher.firstName} {teacher.lastName}</p>
                            <p className="text-xs text-muted-foreground">{teacher.specialization || "معلم"}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="lg:col-span-2">
                    {!selectedTeacherForEvidence ? (
                      <div className="text-center py-12">
                        <FolderOpen className="h-12 w-12 mx-auto mb-3 text-muted-foreground opacity-30" />
                        <p className="text-muted-foreground">اختر معلماً لعرض شواهده</p>
                      </div>
                    ) : principalEvidenceLoading ? (
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between mb-2">
                          <Badge variant="secondary">{principalTeacherWitnesses.length} شاهد</Badge>
                          <h4 className="font-bold text-sm">
                            شواهد {selectedTeacherForEvidence.firstName} {selectedTeacherForEvidence.lastName}
                          </h4>
                        </div>
                        {evaluationItems.map((item) => {
                          const itemWitnesses = principalTeacherWitnesses.filter(w => w.evaluationItemId === item.id);
                          const ItemIcon = getIconComponent(item.icon);
                          return (
                            <Card key={item.id} className="overflow-visible" data-testid={`principal-evidence-item-${item.id}`}>
                              <div className="p-3 flex items-center justify-between gap-3 border-b">
                                <Badge variant="secondary" className="text-xs">{itemWitnesses.length} شاهد</Badge>
                              ...
                              {itemWitnesses.length === 0 ? (
                                <div className="p-3 text-center text-xs text-muted-foreground">لا توجد شواهد</div>
                              ) : (
                                <div className="p-2 space-y-1.5">
                                  {itemWitnesses.map((w) => (
                                    <div key={w.id} className="flex items-center justify-between gap-2 p-2.5 rounded-lg bg-muted/30" data-testid={`principal-witness-${w.id}`}>
                                      <div className="flex gap-1 shrink-0">
                                        {w.link && (
                                          <Button size="icon" variant="ghost" onClick={() => window.open(w.link!, "_blank")}>
                                      ...
                                      <div className="flex items-center gap-2 flex-1 justify-end min-w-0">
                                        <div className="text-right min-w-0">
                                          <p className="font-medium text-xs truncate">{w.title}</p>
                                          <div className="flex items-center gap-1 justify-end">
                                            <Badge variant="outline" className="text-[9px]">
                                              {w.type === "file" ? `${w.files?.length || 0} ملفات` : "رابط"}
                                            </Badge>
                                            {w.files && w.files.length > 0 && (
                                              <div className="flex gap-0.5">
                                                {w.files.slice(0, 3).map((f) => (
                                                  <div key={f.id} className="h-4 w-4 rounded bg-muted flex items-center justify-center">
                                                    {f.fileType?.includes("pdf") ? <FileText className="h-2.5 w-2.5 text-red-500" /> :
                                                     f.fileType?.includes("image") || f.fileType?.includes("jpg") || f.fileType?.includes("png") ? <Image className="h-2.5 w-2.5 text-green-500" /> :
                                                     <File className="h-2.5 w-2.5 text-muted-foreground" />}
                                                  </div>
                                                ))}
                                                {w.files.length > 3 && <span className="text-[9px] text-muted-foreground">+{w.files.length - 3}</span>}
                                              </div>
                                            )}
                                          </div>
                                        </div>
                                        <div className={`w-6 h-6 rounded flex items-center justify-center shrink-0 ${w.type === "link" ? "bg-purple-100 dark:bg-purple-900/30" : "bg-blue-100 dark:bg-blue-900/30"}`}>
                                          {w.type === "link" ? <LinkIcon className="h-3 w-3 text-purple-500" /> : <FileText className="h-3 w-3 text-blue-500" />}
                                        </div>
                                      </div>
                                    </div>
                                  ))}
      ...
      <EvidenceReviewModal
        ...
        onApprove={() => {
          ...
          toast({ title: "تم الاعتماد", description: "تم اعتماد الشاهد بنجاح" });
          setShowEvidenceReview(false);
        }}
        onReject={() => {
          toast({ title: "تم الرفض", description: "تم رفض الشاهد", variant: "destructive" });
          setShowEvidenceReview(false);
        }}
      />
    </div>
