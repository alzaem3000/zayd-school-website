...
export default function Home() {
  ...
  const { data: teacherWitnesses = [], isLoading: teacherWitnessesLoading } = useQuery<TeacherWitnessWithFiles[]>({
    queryKey: ["/api/teacher-witnesses"],
  });
  ...
  const goalsTotalWeight = goalIndicators.reduce((sum, i) => sum + (i.weight || 0), 0);
  const competenciesTotalWeight = competencyIndicators.reduce((sum, i) => sum + (i.weight || 0), 0);
  ...
  const deleteTeacherWitnessMutation = useMutation({
    mutationFn: async (witnessId: string) => {
      return apiRequest("DELETE", `/api/teacher-witnesses/${witnessId}`);
    },
    onSuccess: () => {
  ...
  const handleAddWitness = (preItemId?: number) => {
    setPreSelectedItemId(preItemId);
  ...
  const renderIndicatorGrid = (indicatorList: IndicatorWithCriteria[], type: "goal" | "competency") => {
    ...
    return (
      <>
        {renderWeightBar(
          indicatorList.reduce((sum, i) => sum + (i.weight || 0), 0),
          type
        )}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
          {indicatorList.map((indicator) => {
            ...
            return (
              <Card
                key={indicator.id}
                className={`overflow-visible cursor-pointer transition-all hover-elevate ${getStatusBorderClass(indicator)} ${
                  expandedIndicator === indicator.id ? "ring-2 ring-primary shadow-lg" : ""
                }`}
              ...
              >
                        ...
                        <>
                          {indicator.witnesses.slice(0, 5).map((w) => (
                            ...
                            >
                              ...
                              {w.fileType === "link" && <LinkIcon className="h-3.5 w-3.5 text-purple-500" />}
                              {(!w.fileType || (w.fileType !== "pdf" && w.fileType !== "image" && w.fileType !== "video" && w.fileType !== "link")) && <File className="h-3.5 w-3.5 text-muted-foreground" />}
                            </div>
                          ))}
                ...
                <div className="border-t px-5 py-3 bg-muted/20">
                  <Button
                    onClick={(e) => { e.stopPropagation(); handleAddWitness(); }}
                    size="sm"
                  ...
                  <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-2 justify-end mt-1 flex-wrap">
                        ...
                        {indicator.weight ? <Badge variant="outline">{indicator.weight}%</Badge> : null}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {expandedWitnesses.map((witness) => {
                          const iconConfig = witness.fileType === "pdf"
                            ...
                            : witness.fileType === "link"
                            ? { icon: LinkIcon, color: "text-purple-500", bg: "bg-purple-100 dark:bg-purple-900/30", label: "رابط" }
                                ...
                                <div className="flex gap-1 shrink-0">
                                  {witness.link && (
                                    <Button
                                      size="icon"
                                      variant="ghost"
                                      onClick={() => window.open(witness.link!, "_blank")}
                                      data-testid={`button-open-link-witness-${witness.id}`}
                                    >
                                      <ExternalLink className="h-4 w-4" />
                                    </Button>
                                  )}
                                  {witness.fileUrl && !witness.link && (
                                    <Button
                                      size="icon"
                                      variant="ghost"
                                      onClick={() => {
    ...
    <div className="min-h-screen bg-background" dir="rtl" data-testid="page-home">
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6" data-testid="section-stats">
          <Card className="p-4 text-center" style={{ backgroundColor: "#006C35", color: "white" }} data-testid="stat-total-indicators">
            <div className="text-3xl font-bold">{currentStats.totalIndicators}</div>
            <div className="text-sm opacity-90">إجمالي المؤشرات</div>
          </Card>
          <Card className="p-4 text-center" style={{ backgroundColor: "#006C35", color: "white" }} data-testid="stat-completed-indicators">
          ...
          <Card className="p-4 text-center" style={{ backgroundColor: "#006C35", color: "white" }} data-testid="stat-total-witnesses">
            <div className="text-3xl font-bold">{currentStats.totalWitnesses}</div>
            <div className="text-sm opacity-90">إجمالي الشواهد</div>
          </Card>
          <Card className="p-4 text-center" style={{ backgroundColor: "#006C35", color: "white" }} data-testid="stat-completion-percentage">
        ...
        <div className="mb-6" data-testid="section-evidence-dashboard">
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold" style={{ color: "#006C35" }}>شواهد الأداء</h2>
            <p className="text-sm text-muted-foreground mt-1">توثيق الشواهد المرتبطة بعناصر التقييم الوزارية</p>
          </div>
          
          <div className="grid grid-cols-3 gap-4 mb-6">
            <Card className="overflow-visible hover-elevate cursor-pointer" onClick={() => setWitnessUploadOpen(true)} data-testid="card-add-witness">
              <div className="p-5 flex items-center justify-between gap-4">
                <div className="text-right">
                  <h3 className="font-bold text-base mb-1">إضافة شاهد</h3>
                  <p className="text-sm text-muted-foreground">صور أو ملفات</p>
                </div>
                <div className="w-14 h-14 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: "#3B82F6" }}>
                  <Plus className="h-7 w-7 text-white" />
                </div>
              </div>
            </Card>
            <Card className="overflow-visible hover-elevate cursor-pointer" onClick={() => {
              const el = document.querySelector('[data-testid="section-evidence-list"]');
              if (el) el.scrollIntoView({ behavior: "smooth" });
            }} data-testid="card-view-witnesses">
              <div className="p-5 flex items-center justify-between gap-4">
                <div className="text-right">
                  <h3 className="font-bold text-base mb-1">عرض الشواهد</h3>
                  <p className="text-sm font-bold" style={{ color: "#10B981" }}>{teacherWitnesses.length}</p>
                </div>
                <div className="w-14 h-14 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: "#10B981" }}>
                  <FolderOpen className="h-7 w-7 text-white" />
                </div>
              </div>
            </Card>
            <Card className="overflow-visible hover-elevate cursor-pointer" data-testid="card-evaluation-items">
              <div className="p-5 flex items-center justify-between gap-4">
                <div className="text-right">
                  <h3 className="font-bold text-base mb-1">عناصر التقييم</h3>
                  <p className="text-sm font-bold" style={{ color: "#5B5FC7" }}>{evaluationItems.length}</p>
                </div>
                <div className="w-14 h-14 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: "#5B5FC7" }}>
          ...
          <div className="space-y-4" data-testid="section-evidence-list">
            {evaluationItems.map((item) => {
              const itemWitnesses = teacherWitnesses.filter(w => w.evaluationItemId === item.id);
              const ItemIcon = getIconComponent(item.icon);
              return (
                <Card key={item.id} className="overflow-visible" data-testid={`evidence-item-${item.id}`}>
                  <div className="p-4 flex items-center justify-between gap-3 border-b">
                    <Button
                      onClick={() => handleAddWitness(item.id)}
                      size="sm"
                      className="gap-2 shrink-0"
                      style={{ backgroundColor: "#006C35" }}
                      data-testid={`button-add-witness-item-${item.id}`}
                    >
                      <Plus className="h-4 w-4" />
                      إضافة
                    </Button>
                  ...
                  {itemWitnesses.length === 0 ? (
                    <div className="p-4 text-center text-sm text-muted-foreground" data-testid={`no-evidence-${item.id}`}>
                      لا توجد شواهد مرفقة
                    </div>
                  ) : (
                    <div className="p-3 space-y-2">
                      {itemWitnesses.map((witness) => (
                        <div key={witness.id} className="flex items-center justify-between gap-3 p-3 rounded-xl bg-muted/30" data-testid={`witness-row-${witness.id}`}>
                          <div className="flex gap-1 shrink-0">
                            {witness.link && (
                              <Button size="icon" variant="ghost" onClick={() => window.open(witness.link!, "_blank")} data-testid={`button-open-link-${witness.id}`}>
                                <ExternalLink className="h-4 w-4" />
                              </Button>
                            )}
                            <Button
                              ...
                              onClick={() => {
                                if (confirm("هل أنت متأكد من حذف هذا الشاهد؟")) {
                                  deleteTeacherWitnessMutation.mutate(witness.id);
                                }
                              }}
                              data-testid={`button-delete-tw-${witness.id}`}
                            >
                              <Trash2 className="h-4 w-4" />
                            </Button>
                          </div>
                          <div className="flex items-center gap-3 flex-1 justify-end min-w-0">
                            <div className="text-right min-w-0">
                              <p className="font-medium text-sm truncate">{witness.title}</p>
                              <div className="flex items-center gap-2 justify-end flex-wrap">
                                <Badge variant="outline" className="text-[10px]">
                                  {witness.type === "file" ? `${witness.files?.length || 0} ملفات` : "رابط"}
                                </Badge>
                                {witness.files && witness.files.length > 0 && (
                                  <div className="flex gap-1">
                                    {witness.files.slice(0, 4).map((f) => (
                                      <div key={f.id} className="h-5 w-5 rounded bg-muted flex items-center justify-center">
                                        {f.fileType?.includes("pdf") ? <FileText className="h-3 w-3 text-red-500" /> :
                                         f.fileType?.includes("image") || f.fileType?.includes("jpg") || f.fileType?.includes("png") || f.fileType?.includes("jpeg") ? <Image className="h-3 w-3 text-green-500" /> :
                                         f.fileType?.includes("video") ? <Video className="h-3 w-3 text-blue-500" /> :
                                         <File className="h-3 w-3 text-muted-foreground" />}
                                      </div>
                                    ))}
                                    {witness.files.length > 4 && <span className="text-[10px] text-muted-foreground">+{witness.files.length - 4}</span>}
                                  </div>
                                )}
                              </div>
                            </div>
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${witness.type === "link" ? "bg-purple-100 dark:bg-purple-900/30" : "bg-blue-100 dark:bg-blue-900/30"}`}>
                              {witness.type === "link" ? <LinkIcon className="h-4 w-4 text-purple-500" /> : <FileText className="h-4 w-4 text-blue-500" />}
                            </div>
                          </div>
                        </div>
                      ))}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} dir="rtl">
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
              <TabsList className="grid grid-cols-2 w-auto">
                <TabsTrigger value="goals" className="gap-2 px-6" data-testid="tab-goals">
                  <Target className="h-4 w-4" />
                  الأداء الوظيفي (الأهداف)
                </TabsTrigger>
            ...
            <TabsContent value="goals">
              <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                <Button onClick={() => handleAddIndicator("goal")} style={{ backgroundColor: "#006C35" }} className="gap-2" data-testid="button-add-goal">
                  <Plus className="h-4 w-4" />
                  إضافة هدف جديد
                </Button>
                <h3 className="text-lg font-bold" style={{ color: "#006C35" }}>أهداف الأداء الوظيفي</h3>
              </div>
              {renderIndicatorGrid(goalIndicators, "goal")}
            </TabsContent>
      ...
      <WitnessUploadModal
        ...
        onClose={() => { setWitnessUploadOpen(false); setPreSelectedItemId(undefined); }}
